...
function GEOdeclined(error) {
  document.getElementById('adresse').innerHTML = 'Error: ' + error.message;
}

if (navigator.geolocation) {
	navigator.geolocation.getCurrentPosition(GEOprocess, GEOdeclined);
}else{
  document.getElementById('adresse').innerHTML = 'Your browser sucks. Upgrade it.';
}
...
function updatePage() {
 if (xmlHttp.readyState == 4) {
  ...
  document.getElementsByName("adresse")[0].value = '' + response;
  document.getElementsByName("adresse")[1].value = '' + response;
 }
}
