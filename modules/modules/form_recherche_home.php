                        ...
                        <div class="choix">
                       	  <p>
                        	  ...
                        	  <label>
                        	    ...
                        	    Coupons</label>
                                <label>
                        	    <input type="radio" name="choix" value="3" id="choix_2" <?php if(strpos($_SERVER['PHP_SELF'],'rechercher2.php') !== FALSE) echo 'checked="checked"'; ?> onclick="changeAction('rechercher2.php');" />
                        	    Magasins</label>
                        	  <br />
                      	  </p>
                        </div>
...
<input type="text" name="adresse" id="adresse" value="<?php if(isset($_GET['adresse']) and !empty($_GET['adresse']))  echo $_GET['adresse']; else echo "Entrer votre adresse"; ?>" onblur="if(this.value == '') this.value='Entrer votre adresse';" onfocus="if(this.value == 'Entrer votre adresse') this.value=''" class="width1" > 

<select name="rayon" class="width2">
<option value="999">Rayon</option>
...
<?php while($row_categories = mysql_fetch_assoc($categories)) { ?>
  <option value="<?php echo $row_categories['cat_id']?>" <?php if(isset($_GET['categorie']) and $_GET['categorie'] == $row_categories['cat_id']) echo "SELECTED"; ?>><?php echo (htmlspecialchars($row_categories['cat_name'])); ?></option>
  <?php
} 
?>
</select>
...
<input type="button" value="" onclick="if(document.getElementById('mot_cle').value == 'Mot clé') document.getElementById('mot_cle').value=''; if(document.getElementById('adresse').value == 'Entrer votre adresse') document.getElementById('adresse').value=''; this.form.submit();" class="search_ok" />
</form>
                    </div>
                    </div>
                </div>