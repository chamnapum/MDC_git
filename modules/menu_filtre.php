...
<div style="float:left; width:310px;">
    ...
    foreach($map2_result as $cat2){
		...
		if($cat2['parent_id']=='0'){
            ...
            echo'<li class="hide1 toutcat" style="background:none; padding:0; margin-top:10px;">'.$listImg.'<a href="javascript:location=\'sub_produits-'.$nom_region.'-'.$default_region.''.$lien.'\'">' .$cat2['cat_name'].'</a></li>';
        }   
        ...
        if($cat2['parent_id']==$_GET['categorie']){
            ...
            echo'<li class="hide2 niv_1 toutcat" style="background:none; padding:0; margin-bottom: 5px !important;">'.$listImg.'<a href="javascript:location=\'sub_sub_produits-'.$nom_region.'-'.$default_region.''.$lien.'\'">' .$cat2['cat_name'].'</a></li>';
            ?>
            <style>
                .hide1{
        ...
        if($cat2['parent_id']==$_GET['sous_categorie']){
            ...
            echo'<li class="hide3 niv_1 toutcat" style="background:none; padding:0;">'.$listImg.'<a href="javascript:location=\'sub_sub_sub_produits-'.$nom_region.'-'.$default_region.''.$lien.'\'">' .$cat2['cat_name'].'</a></li>';
            ?>
            <style>
                .hide2{
                    display:none !important;
	...
	$query_coupon = "SELECT 
					...
					FROM
						...
						WHERE magazins.region = '".$default_region."' 
						  AND produits.en_avant = '1' 
						  AND produits.en_avant_payer = '1' 
						  AND produits.en_avant_fin >= '".$date."' 
						  AND DATE_ADD(
							produits.en_avant_fin,
							INTERVAL - produits.day_en_avant DAY
						  ) <= '".$date."' 
						  AND (
							produits.activate = '1' 
							OR (
...
?>
	...
	<div class="km" style="margin:0px 20px 20px 20px;">
		<div class="ad">
			...
			<img src="timthumb.php?src=assets/images/produits/<?php echo $row_coupon['photo1'] ?>&amp;w=37&amp;h=37&z=1">  
			<?php }?> 
		</div>
		<div class="ad_title">
