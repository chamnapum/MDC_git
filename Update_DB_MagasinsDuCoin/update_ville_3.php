...
// start execut time
 ...
 $starttime = $mtime;
//Update Ville
$query = mysql_query("select distinct(ville_lower), code_postal from magazins_r where id_ville = 0 order by RAND() limit 1000");
$count = 0;
while($row = mysql_fetch_array($query)){
	$id_ville = $row['id_ville'];
	$ville_lower = $row['ville_lower'];
	$code_postal = intval($row['code_postal']);
	...
	$sql = "UPDATE magazins_r SET id_ville=(select v.id_ville from maps_ville2 v where nom_lower = '$ville_lower' and cp  LIKE '".substr($code_postal,0,1)."%' limit 1) where ville_lower = '$ville_lower' and code_postal = '$code_postal'";
