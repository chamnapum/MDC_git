...
FROM (((coupons  
LEFT JOIN magazins ON magazins.id_magazin=coupons.magasin_default)
LEFT JOIN category ON category.cat_id=coupons.categories)
LEFT JOIN maps_ville ON maps_ville.id_ville=magazins.ville)";*/
$query_liste_produit = "SELECT magazins.nom_magazin, magazins.logo, magazins.adresse, magazins.heure_ouverture, magazins.jours_ouverture, magazins.latlan, magazins.photo1, coupons.date_debut, coupons.date_fin, coupons.titre, coupons.min_achat, coupons.description, coupons.id_coupon, magazins.region, category.cat_name, magazins.id_magazin, maps_ville.nom AS ville, (SELECT COUNT(*)  FROM produits WHERE id_magazin = magazins.id_magazin) AS nb_produits 
FROM (((coupons
LEFT JOIN magazins ON magazins.id_magazin=coupons.magasin_default)
LEFT JOIN category ON category.cat_id=coupons.categories)
LEFT JOIN maps_ville ON maps_ville.id_ville=magazins.ville)";
...
$tab = array();
						...
						if(isset($_GET['sous_categorie'])){
							$tab[] = " coupons.sous_categorie = '".$sous_categorie."'";
						}
						elseif(isset($_GET['categorie'])){
							$tab[] = " coupons.categories  = '".$categorie."'";
						}
...
$query_liste_produit .= " ORDER BY en_tete_liste DESC, date_debut DESC";
...
?>
 ...
 <?php echo str_replace('-',' '.$xml->A.' ',$liste['heure_ouverture']); ?></div>
                 ...
                 <div class="box_img"> 
                 	<?php echo "<a href=\"detail_magasin.php?mag_id=".$liste['id_magazin']."&#tabs-5\" target=\"_top\">"?>
                ...
                <div class="box_desc" >
                     ...
                     <span class="magazin"><a href="rechercher.php?adresse=&rayon=999&mot_cle=&categorie=&sous_categorie=&magasin=<?php echo $liste['id_magazin'];  ?>" style="color: #F6AE30;"><?php echo $liste['nb_produits'];?> <?php echo $xml->produits_disponibles_dans_ce_magasin ; ?></a></span> 
                      <div class="prix_inner">
                              <div class="box_prix">
                              	<span class="prix">
								<?php /*?><?php echo $liste['reduction']." %"; ?><?php */?>
                                </span><br />
        ...
        <div class="box_event"> 
            <div class="box_evnt" style="width:176px; margin-right:0"><p><a href="javascript:;" onClick="ajax('ajax/addtocart.php?id_coupon=<?php echo $liste['id_coupon']; ?>','#cart');" id="<?php echo $liste['id_coupon'];?>"><?php echo $xml->Ajouter_au_panier ; ?></a> </p></div><br />
                ...
                <div class="box_desc" >
					 ...
					 <?php echo $xml->produits_disponibles_dans_ce_magasin ; ?>:</span> 
                      <div class="prix_inner">
                              <div class="box_prix">
                              	<span class="prix">
								<?php /*?><?php echo $l[1]['reduction']." %"; ?><?php */?>
                                </span><br />
