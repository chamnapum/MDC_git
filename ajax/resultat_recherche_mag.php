...
$date = date('Y-m-d');
	...
	$query_liste_produit_test = " ( SELECT maps_ville.cp,
	...
	$query_liste_produit_test .= " AND magazins.en_tete_liste = '1' 
									  AND magazins.en_tete_liste_payer = '1' 
									  AND magazins.en_tete_liste_fin > '2014-09-13 05:22:57' 
									  AND DATE_ADD(
										en_tete_liste_fin,
										INTERVAL - day_en_tete_liste DAY
									  ) <= '2014-09-13 05:22:57'
										AND magazins.activate='1' AND magazins.payer='1' AND (
										magazins.approuve = '1' 
										OR (
										  magazins.approuve = 0 
	...
	$query_liste_produit_test .= " AND magazins.activate='1' AND magazins.payer='1' AND (
									   ...
									   AND (
										magazins.en_tete_liste != '1' 
										OR magazins.en_tete_liste_payer != '1' 
										OR magazins.en_tete_liste_fin IS NULL 
										OR day_en_tete_liste IS NULL 
										OR DATE_ADD(
										  en_tete_liste_fin,
										  INTERVAL - day_en_tete_liste DAY
										) > '".$datetime."' 
										OR magazins.en_tete_liste_fin <= '".$datetime."'
									  ) 
	...
	$query_count .= "SELECT COUNT(*) AS total 
			...
			";
	$query_count .= $where;
	$query_count .= " AND magazins.activate='1' AND magazins.payer='1' AND (
						magazins.approuve = '1' 
						OR (
						  magazins.approuve = 0 
						  AND magazins.public = 1 
	...
	$resutl_count = mysql_query($query_count);
	$nbr_totals	= mysql_fetch_array($resutl_count);

	$nbr_total = $nbr_totals['total'];
	...
	if($_GET['pa']=='' || $_GET['pa']=='1'){
		...
		$limit_2 = $limit*$page;
		$limit_1 = ($limit*$page)-1;
		$start_1 = ($limit*$page)-$limit;
	}
	$query_liste_produits=$query_liste_produit_test."  LIMIT 0, $limit_1 ) LIMIT $start_1, $limit_2";
